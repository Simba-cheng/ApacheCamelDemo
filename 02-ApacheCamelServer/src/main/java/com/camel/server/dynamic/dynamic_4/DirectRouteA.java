...
public class DirectRouteA extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		from("jetty:http://0.0.0.0:8282/directMain").setExchangePattern(ExchangePattern.InOnly).recipientList().jsonpath("$.data.routeName").delimiter(",").end().process(new Processor() {
